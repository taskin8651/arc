...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.galleries.update", [$gallery->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="descrption">{{ trans('cruds.gallery.fields.descrption') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('descrption') ? 'is-invalid' : '' }}" name="descrption" id="descrption">{!! old('descrption', $gallery->descrption) !!}</textarea>
                @if($errors->has('descrption'))
                    <div class="invalid-feedback">
                        {{ $errors->first('descrption') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.gallery.fields.descrption_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="image_1">{{ trans('cruds.gallery.fields.image_1') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.gallery.fields.image_1_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($gallery) && $gallery->image_1)
      var file = {!! json_encode($gallery->image_1) !!}
