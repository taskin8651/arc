    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.services.update", [$service->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('short_description') ? 'is-invalid' : '' }}" name="short_description" id="short_description">{{ old('short_description', $service->short_description) }}</textarea>
            ...
            <div class="form-group">
                <label for="price_range">{{ trans('cruds.service.fields.price_range') }}</label>
                <input class="form-control {{ $errors->has('price_range') ? 'is-invalid' : '' }}" type="text" name="price_range" id="price_range" value="{{ old('price_range', $service->price_range) }}">
                @if($errors->has('price_range'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price_range') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.service.fields.price_range_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="add_pdf">{{ trans('cruds.service.fields.add_pdf') }}</label>
                <div class="needsclick dropzone {{ $errors->has('add_pdf') ? 'is-invalid' : '' }}" id="add_pdf-dropzone">
                </div>
                @if($errors->has('add_pdf'))
                    <div class="invalid-feedback">
                        {{ $errors->first('add_pdf') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.service.fields.add_pdf_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="add_doc">{{ trans('cruds.service.fields.add_doc') }}</label>
                <div class="needsclick dropzone {{ $errors->has('add_doc') ? 'is-invalid' : '' }}" id="add_doc-dropzone">
                </div>
                @if($errors->has('add_doc'))
                    <div class="invalid-feedback">
                        {{ $errors->first('add_doc') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.service.fields.add_doc_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="gallery_1">{{ trans('cruds.service.fields.gallery_1') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.service.fields.gallery_1_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="gallery_2">{{ trans('cruds.service.fields.gallery_2') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.service.fields.gallery_2_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    Dropzone.options.addPdfDropzone = {
    url: '{{ route('admin.services.storeMedia') }}',
    maxFilesize: 20, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    ...
    success: function (file, response) {
      $('form').find('input[name="add_pdf"]').remove()
      $('form').append('<input type="hidden" name="add_pdf" value="' + response.name + '">')
    },
    removedfile: function (file) {
      file.previewElement.remove()
      if (file.status !== 'error') {
    ...
    init: function () {
@if(isset($service) && $service->add_pdf)
      var file = {!! json_encode($service->add_pdf) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="add_pdf" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
     error: function (file, response) {
...
<script>
    Dropzone.options.addDocDropzone = {
    url: '{{ route('admin.services.storeMedia') }}',
    maxFilesize: 20, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    ...
    success: function (file, response) {
      $('form').find('input[name="add_doc"]').remove()
      $('form').append('<input type="hidden" name="add_doc" value="' + response.name + '">')
    },
    removedfile: function (file) {
      file.previewElement.remove()
      if (file.status !== 'error') {
    ...
    init: function () {
@if(isset($service) && $service->add_doc)
      var file = {!! json_encode($service->add_doc) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="add_doc" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
     error: function (file, response) {
...
<script>
    ...
    init: function () {
@if(isset($service) && $service->gallery_1)
      var file = {!! json_encode($service->gallery_1) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
...
<script>
    ...
    init: function () {
@if(isset($service) && $service->gallery_2)
      var file = {!! json_encode($service->gallery_2) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
